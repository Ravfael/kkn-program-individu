...
export function MarketplaceFooter() {
  ...
  const waUrl = `https://wa.me/${waNumber}?text=${encodeURIComponent(
    "Halo Tim Posko KKN Desa Candisari, saya ingin bertanya mengenai program pembinaan Pasar UMKM."
  )}`;
    ...
    <footer className="bg-white text-slate-700 border-t border-market-primary/15 font-poppins mt-16">
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12 pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="font-extrabold text-lg text-market-primary leading-tight tracking-tight">
                  Pasar UMKM
                </span>
                <span className="text-xs font-bold text-market-secondary tracking-wide">
                  Desa Candisari
                </span>
              </div>
            </Link>
            ...
            <p className="text-xs text-slate-600 leading-relaxed max-w-sm">
              Etalase produk UMKM unggulan warga Desa Candisari, dibina secara berkelanjutan melalui program pengabdian masyarakat KKN.
            </p>
          </div>

          {/* Column 2: Categories */}
          <div className="flex flex-col gap-3">
            <h4 className="text-xs font-extrabold uppercase tracking-wider text-market-primary border-b border-market-primary/15 pb-1.5 w-fit">
              Kategori Produk
            </h4>
            <ul className="flex flex-col gap-2 text-xs font-semibold text-slate-600">
              {categories.map((cat) => (
                <li key={cat}>
                  <Link
                    href="/marketplace"
                    className="hover:text-market-primary transition-colors flex items-center gap-1.5"
                  >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              <Link
                href="/id"
                className="text-xs font-bold text-market-secondary hover:text-market-primary transition-colors flex items-center gap-1.5 w-fit"
              >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-medium text-slate-500">
          <p>
            &copy; {new Date().getFullYear()} Pasar UMKM Desa Candisari — Program KKN.
          </p>
          <p className="flex items-center gap-1">
            <span>Dibuat dengan</span>
            <Heart className="w-3.5 h-3.5 text-market-primary fill-current inline-block" />
            <span>oleh Tim Mahasiswa KKN Universitas</span>
          </p>
        </div>
