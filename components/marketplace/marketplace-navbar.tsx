...
export function MarketplaceNavbar({ selectedCategory = "Semua", onSelectCategory, searchQuery = "", onSearchChange }: MarketplaceNavbarProps) {
  const categories = ["Semua", "Makanan & Camilan", "Perikanan & Pangan", "Jasa & Layanan"];
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-market-bg/95 backdrop-blur-md border-b border-market-primary/20 shadow-xs font-poppins">
      ...
      <div className="bg-market-primary text-white text-xs font-medium py-1.5 px-4 text-center flex items-center justify-center gap-2">
        ...
        <span>Pesan langsung dari warga Desa Candisari via WhatsApp!</span>
      </div>
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3.5">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center justify-between w-full md:w-auto">
            <Link href="/marketplace" className="flex items-center gap-3 group focus:outline-hidden">
              ...
              <div className="flex flex-col">
                <span className="font-extrabold text-xl text-market-primary leading-tight tracking-tight">Pasar UMKM</span>
                <span className="text-xs font-semibold text-market-secondary tracking-wide">Desa Candisari</span>
              </div>
            </Link>

            {/* Back to main site link (Mobile) */}
            <Link href="/id" className="md:hidden inline-flex items-center gap-1 text-xs font-semibold text-market-secondary hover:text-market-primary bg-white px-3 py-1.5 rounded-full border border-market-secondary/20 shadow-2xs">
          ...
          <div className="w-full md:max-w-md relative flex items-center">
            ...
            <input
              ...
              placeholder="Cari cilor, tiwul, laundry, service komputer, ikan..."
            ...
            />
            {searchQuery && (
              <button onClick={() => onSearchChange?.("")} className="absolute right-3 p-1 rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors" aria-label="Clear search">
          ...
          {categories.map((category) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all duration-150 cursor-pointer ${
                selectedCategory === category ? "bg-market-primary text-white shadow-xs" : "bg-white text-slate-700 hover:bg-market-primary/10 border border-market-primary/15"
              }`}
            >
              {category}
            </button>
